...
const serviceUrlMap = new Map([
    ["MathJax", "./Services/MathJax.js"],
    ["Mathlive", "./Services/Mathlive.js"],
]);
function getServiceUrl(serviceName, options) {
    return new URL(
        serviceUrlMap.get(serviceName) ?? `./Services/${serviceName}.js`,
        options?.baseURL ?? window.location.href
    );
}
const localResourceMap = new Map([
    ["MathJax-config", "/lib/mathjax-default.js"],
    ["InteractJS", "/node_modules/interactjs/dist/interact.min.js"],
    ["MathJax-OpenMiddle-config", "/lib/mathjax-openmiddle.js"],
    ...
    [
        ...
        "/node_modules/@cortex-js/compute-engine/dist/compute-engine.min.esm.js?module",
    ...
    ["SVGdotJS", "/node_modules/@svgdotjs/svg.js/dist/svg.js"],
]);
const remoteResourceMap = new Map([
    ["SVGdotJS", "https://unpkg.com/@svgdotjs/svg.js@3.2.0/dist/svg.js"],
...
function zip(x, y) {
    return range(0, Math.max(x.length, y.length)).map(function (index) {
...
function loadStylesheet(url, options) {
    return new Promise(function (resolve) {
        ...
        link.type = "text/css";
        link.rel = "stylesheet";
        link.href = new URL(`${url}`, options?.baseURL ?? window.location.href);
...
function getResourceUrl(resourceName, options) {
    const resourcePath = getResourcePath(resourceName);
    if (resourcePath === undefined) {
        console.error(
            `A URL for a resource could not be found: ${resourceName}`
        );
        return undefined;
    }
    return new URL(resourcePath, options?.baseURL ?? window.location.href);
}
...
function loadService(serviceName, options = {}) {
    return import(getServiceUrl(serviceName, options));
}
function loadResource(resourceName, options, dynamicImport = true) {
    ...
    var resourcePromise;
    if (dynamicImport) {
        resourcePromise = import(getResourceUrl(resourceName, options));
    } else {
        resourcePromise = loadScript(
            getResourceUrl(resourceName, options),
            options
        );
    }
    return resourcePromise.then(function (result) {
